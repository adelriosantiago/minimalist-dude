	...
	*/
	...
	$.BookBlock.prototype = {
		...
		_init: function(options) {
            ...
            var index = _.findIndex(this.$items, {'id' : hash.substring(1)});
            //Select the starting page
            if (index != -1) {
                this.current = index;
            } else {
                this.current = 0;
                var newHash = '#' + this.$items[0].getAttribute('id');
                //window.location.hash = newHash; //Works but changes the browser history
		...
		_navigate: function(dir, page) {
            ...
            var newHash = '#' + this.$items[this.current].getAttribute('id');
            //window.location.hash = newHash; //Works but changes the browser history
            window.location.replace(newHash);
