(function ( $ ) {
	...
	$.fn.progressTracker = function( options ) {
        ...
        /*$('body').append('<div class="progress-tracker-text"><ul></ul></div>');*/
		...
		$('.pt-section').each(function(index) {
			var $this = $(this),
                ...
                customContent = $this.find('.pt-custom-content').html(),
				itemLink = "",
				itemDescription = "";
        
            if(settings.linking) {
				itemLink = '<a class="pt-circle" href="#' + sectionId + '">' + customContent + '</a>';
			}
			if(settings.tooltip) {
			...
			$('.progress-tracker ul').append(
				'<li class="section-' + sectionId + '" style="margin-bottom: ' + settings.bulletSeparation + 'px;">' + itemLink + itemDescription + '</li>'
    		);
            
            /*$('.progress-tracker-text ul').append(
				'<li>' + 'test' +'</li>'
    		);*/
